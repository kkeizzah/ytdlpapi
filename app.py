...
URL = 'https://ytdlpapi-dusn.onrender.com'
...
def download_audio():
    ...
    try:
        with YoutubeDL(ydl_opts) as ydl:
            ...
            safe_filename = secure_filename(title).replace(' ', '_') + '_by_keithapis' + '.mp3'
            ...
            response = {
                ...
                'result': {
                    ...
                    'stream_url': f'{URL}/stream/{safe_filename}',
                    'download_url': f'{URL}/download/{safe_filename}'
                }
            }
...
def download_video():
    ...
    try:
        with YoutubeDL(ydl_opts) as ydl:
            ...
            safe_filename = secure_filename(title).replace(' ', '_') + f'_{format}' + '_by_apiskeith' + '.mp4'
            output_path = os.path.join(DOWNLOAD_FOLDER, safe_filename)
